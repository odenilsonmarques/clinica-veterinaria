            ...
            @if ($vacinacoes->isEmpty())
                <div class="alert alert-info d-flex flex-column align-items-center py-4 mt-3">
                    ...
                    <div class="d-flex gap-2">
                        <a href="{{ route('vacinacoes.index') }}" class="btn btn-secondary">
                            Voltar para lista de vacinações
                        </a>
                    </div>
                </div>
            @else
                    ...
                    <form method="GET" action="" class="d-flex gap-2">
                        <div class="input-group">
                            <input type="text" name="search" class="form-control"
                                placeholder="Buscar por pet, vacina ou veterinário" value="{{ request('search') }}">
                            <button type="submit"
                        ...
                        <a href="{{ route('vacinacoes.index') }}" class="btn btn-outline-secondary">
                            Limpar
                        </a>
                    </form>
            ...
            <table class="table   table-hover caption-top">
                ...
                <tbody>
                    @foreach ($vacinacoes as $vacinacao)
                        <tr>
                            <td>{{ $vacinacao->pet->nome }}</td>
                            <td>{{ $vacinacao->vacina->nome }}</td>
                            <td>{{ $vacinacao->veterinario->nome }}</td>
